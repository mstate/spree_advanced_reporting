module Spree
  class AdvancedReport
    ...
    def revenue(order)
      rev = order.line_items.inject(0) { |a, b| a += b.quantity * (b.price / (1 + (b.try(:tax_category).try(:tax_rates).try(:first).try(:amount) || b.try(:variant).try(:product).try(:tax_category).try(:tax_rates).try(:first).try(:amount) || 0 ))) }
      if !self.product.nil? && product_in_taxon
        rev = order.line_items.select { |li| li.product == self.product }.inject(0) { |a, b| a += b.quantity * (b.price / (1 + (b.try(:tax_category).try(:tax_rates).try(:first).try(:amount) || b.try(:variant).try(:product).try(:tax_category).try(:tax_rates).try(:first).try(:amount) || 0 ))) }
      elsif !self.taxon.nil?
        rev = order.line_items.select { |li| li.product && li.product.taxons.include?(self.taxon) }.inject(0) { |a, b| a += b.quantity * (b.price / (1 + (b.try(:tax_category).try(:tax_rates).try(:first).try(:amount) || b.try(:variant).try(:product).try(:tax_category).try(:tax_rates).try(:first).try(:amount) || 0 ))) }
      end
    ...
    def tax(order)
      ...
      Spree::TaxCategory.all.each do |c|
        tax["#{c.name}"] ||= 0
        tax["#{c.name}"] += order.line_items.select{|li| (li.tax_category == c) || (li.variant.tax_category == c)}.inject(0) { |tax, li| tax += li.quantity * (li.price - (li.price / (1+ c.tax_rates.first.amount)))}

        tax["total_#{c.name}"] ||= 0
        tax["total_#{c.name}"] += order.line_items.select{|li| (li.tax_category == c) || (li.variant.tax_category == c)}.inject(0) { |tax, li| tax += li.quantity * li.price}
      end

      if !self.product.nil? && product_in_taxon
        Spree::TaxCategory.all.each do |c|
          tax["#{c.name}"] ||= 0
          tax["#{c.name}"] += order.line_items.select { |li| li.product == self.product }.select{|li| (li.tax_category == c) || (li.variant.tax_category == c)}.inject(0) { |tax, li| tax += li.quantity * (li.price - (li.price / (1+ c.tax_rates.first.amount)))}

          tax["total_#{c.name}"] ||= 0
          tax["total_#{c.name}"] += order.line_items.select { |li| li.product == self.product }.select{|li| (li.tax_category == c) || (li.variant.tax_category == c)}.inject(0) { |tax, li| tax += li.quantity * li.price}
        end
      elsif !self.taxon.nil?
        Spree::TaxCategory.all.each do |c|
          tax["#{c.name}"] ||= 0
          tax["#{c.name}"] += order.line_items.select { |li| li.product && li.product.taxons.include?(self.taxon) }.select { |li| li.product == self.product }.select{|li| (li.tax_category == c) || (li.variant.tax_category == c)}.inject(0) { |tax, li| tax += li.quantity * (li.price - (li.price / (1+ c.tax_rates.first.amount)))}

          tax["total_#{c.name}"] ||= 0
          tax["total_#{c.name}"] += order.line_items.select { |li| li.product && li.product.taxons.include?(self.taxon) }.select { |li| li.product == self.product }.select{|li| (li.tax_category == c) || (li.variant.tax_category == c)}.inject(0) { |tax, li| tax += li.quantity * li.price}
        end
      end
    ...
    def profit(order)
      profit = order.line_items.inject(0) { |p, li| p += ((li.price / (1 + (li.try(:tax_category).try(:tax_rates).try(:first).try(:amount) || li.try(:variant).try(:product).try(:tax_category).try(:tax_rates).try(:first).try(:amount) || 0))) - ((li.cost_price || li.variant.cost_price || li.variant.product.master.cost_price.to_f) / (1 + (li.try(:tax_category).try(:tax_rates).try(:first).try(:amount) || li.try(:variant).try(:product).try(:tax_category).try(:tax_rates).try(:first).try(:amount) || 0))))*li.quantity }
      if !self.product.nil? && product_in_taxon
        profit = order.line_items.select { |li| li.product == self.product }.inject(0) { |p, li| p += ((li.price / (1 + (li.try(:tax_category).try(:tax_rates).try(:first).try(:amount) || li.try(:variant).try(:product).try(:tax_category).try(:tax_rates).try(:first).try(:amount) || 0))) - ((li.cost_price || li.variant.cost_price || li.variant.product.master.cost_price.to_f) / (1 + (li.try(:tax_category).try(:tax_rates).try(:first).try(:amount) || li.try(:variant).try(:product).try(:tax_category).try(:tax_rates).try(:first).try(:amount) || 0))))*li.quantity }
      elsif !self.taxon.nil?
        profit = order.line_items.select { |li| li.product && li.product.taxons.include?(self.taxon) }.inject(0) { |p, li| p += ((li.price / (1 + (li.try(:tax_category).try(:tax_rates).try(:first).try(:amount) || li.try(:variant).try(:product).try(:tax_category).try(:tax_rates).try(:first).try(:amount) || 0))) - ((li.cost_price || li.variant.cost_price || li.variant.product.master.cost_price.to_f) / (1 + (li.try(:tax_category).try(:tax_rates).try(:first).try(:amount) || li.try(:variant).try(:product).try(:tax_category).try(:tax_rates).try(:first).try(:amount) || 0))))*li.quantity }
      end
